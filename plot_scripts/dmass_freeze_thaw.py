...
for criteria in criterium:
    ...
    if nety == 'nexrad':
        ...
        if nexadj > 1.0:
            ...
            ax1.set_ylabel('Cumulative spatial average of\n' + nety_name + ' gage precipitation (inches)')
    if gadj > 1.0:
        ax1.set_xlabel('Cumulative spatial average of adjusted\n' + netx_name + ' gage precipitation (inches)')
    else:
        ax1.set_xlabel('Cumulative spatial average of\n' + netx_name + ' gage precipitation (inches)')
    ######### Add Title ###########
    title_text = ''
    ...
    # revised titles to use 'en dash' to seperate dates (u"\u2013")
    if criteria == 'Thaw':
        ...
        plt.title('Nonfreezing days, ' + str(start.month)+'/'+str(start.day)+'/'+str(start.year) + u"\u2013" + str(finish.month)+'/'+str(finish.day)+'/'+str(finish.year),fontsize='13',y=1.006) #\n' + str(start)[:10] + ' -- ' + str(finish)[:10]  + ' (' + str(len(in_gage)) + ' days)')
    if criteria == 'Freeze':
        ...
        plt.title('Freezing days, ' + str(start.month)+'/'+str(start.day)+'/'+str(start.year) + u"\u2013" + str(finish.month)+'/'+str(finish.day)+'/'+str(finish.year),fontsize='13',y=1.006) # + str(start)[:10] + ' -- ' + str(finish)[:10] + ' (' + str(len(in_gage)) + ' days)')
    ax1.grid(True)
