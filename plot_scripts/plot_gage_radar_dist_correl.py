...
for gage_type in gage_types:
    ...
    ############# Search through radar distance txt file  ####################
    fradar = open(maindir + 'data\\INPUT_gages_radar_distance.txt','r')
    radar_lib = {}
    for astation in fradar:
        if astation[:4] != 'Stat':
            idinfo = astation.split('\t')
            station_id = idinfo[1].rstrip()
            radar_dist = idinfo[2].rstrip()
                ...
                if line[:2] == '20': #ignores header
                    ...
                    if date_check in temp_lib[criteria]:
                        ...
                        if pgage != 'na' and pnex != 'na':
                            key = str(each_pair)[:-4]
                            if key in gage_lib:
                                gage_lib[key].append(float(pgage)*padj)
                            else:
                                gage_lib[key]=[float(pgage)*padj]
                            if key in nex_lib:
    ...
    for stations in gages_met:
        ...
        ################### Add gage number to point labels ######################
    ...
    if padj != 1.0:
        ax1.set_ylabel('((Total adjusted NEXRAD'+ u"\u2013" +'MPE - total adjusted ' + gage_net.upper() + ' gage)/\ntotal adjusted ' + gage_net.upper() + ' gage) x 100', fontsize=10)
    else:
        ax1.set_ylabel('((Total NEXRAD'+ u"\u2013" +'MPE - total ' + gage_net.upper() + ' gage)/\ntotal ' + gage_net.upper() + ' gage) x 100', fontsize=10)
    ...
    plt.savefig(maindir + '\\figures\\final\\' + criteria.lower() + '_days\\' + gage_net + '_gages\\' + gage_type + '_gage_radar_correl_' + criteria + '_days_padj.png', dpi=150, bbox_inches='tight')
