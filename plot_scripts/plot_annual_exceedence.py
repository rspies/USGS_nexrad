...
for exceed_prob in exceed_prob_list:
    ...
    if gage_net == 'usgs' and padj != 1.0:
        ax1.plot(years_plot, in_gage, ls='-', color='blue', lw=1.75, label = str('Adjusted ' + gage_name + ' Gages (PADJ=' + str(padj) +')'))
    else:
        ax1.plot(years_plot, in_gage, ls='-', color='blue', lw=1.75, label = str(gage_name + ' Gages'))
    if padj != 1.0:
        ax1.plot(years_plot, in_nex, ls='--', color='red', lw=1.75, label = 'Adjusted NEXRAD-MPE (PADJ=' + str(padj) +')')
    else:
        ax1.plot(years_plot, in_nex, ls='--', color='red', lw=1.75, label = 'NEXRAD-MPE')
    ...
    if padj > 1.0:
        plt.savefig(maindir + '\\figures\\final\\' + criteria.lower() + '_days\\precip_quantiles\\' + gage_net + '_' + str(int(exceed_prob*100)) + '_percent_exceedance_padj.png', dpi=150, bbox_inches='tight')
    else:
        plt.savefig(maindir + '\\figures\\final\\' + criteria.lower() + '_days\\precip_quantiles\\' + gage_net + '_' + str(int(exceed_prob*100)) + '_percent_exceedance.png', dpi=150, bbox_inches='tight')
